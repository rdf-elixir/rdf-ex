%% Grammar for TriG as specified in https://www.w3.org/TR/rdf12-trig/

Nonterminals trigDoc
  block triplesOrGraph wrappedGraph triples2 triplesBlock labelOrSubject
...
Terminals prefix_ns prefix_ln iriref blank_node_label anon
  ...
  '.' ';' ',' '[' ']' '(' ')' '{' '}' '^^' '@prefix' '@base' 'PREFIX' 'BASE' 'GRAPH' 'a'
  '<<' '>>' '{|' '|}' .

Rootsymbol trigDoc.


trigDoc -> directive : [{directive, '$1'}] .
trigDoc -> block : ['$1'] .
trigDoc -> directive trigDoc : [{directive, '$1'} | '$2'] .
trigDoc -> block trigDoc : ['$1' | '$2'] .

block -> triplesOrGraph                      : '$1' .
block -> triples2                            : {triples, '$1'} .
block -> wrappedGraph                        : {graph, nil, '$1'} .
block -> 'GRAPH' labelOrSubject wrappedGraph : {graph, '$2', '$3'} .

triplesOrGraph -> labelOrSubject wrappedGraph             : {graph, '$1', '$2'} .
triplesOrGraph -> labelOrSubject predicateObjectList '.'  : {triples, { '$1', '$2' }} .
triplesOrGraph -> quotedTriple predicateObjectList '.'    : {triples, { '$1', '$2' }} .

triples2 -> blankNodePropertyList '.'                     : '$1' .
triples2 -> blankNodePropertyList predicateObjectList '.' : { '$1', '$2' } .
triples2 -> collection predicateObjectList '.'            : { '$1', '$2' } .

wrappedGraph -> '{' '}'              : [] .
wrappedGraph -> '{' triplesBlock '}' : '$2' .

triplesBlock -> triples                   : [{triples, '$1'}] .
triplesBlock -> triples '.'               : [{triples, '$1'}] .
triplesBlock -> triples '.' triplesBlock  : [{triples, '$1'} | '$3'] .

labelOrSubject -> iri       : '$1' .
labelOrSubject -> blankNode : '$1' .
